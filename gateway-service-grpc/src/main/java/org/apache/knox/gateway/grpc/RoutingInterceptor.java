 ...
 * available here: gRPC fixes request paths at {@code /pkg.Service/Method}, and
 * connection strings for such protocols typically forbid a path component
 * altogether. The topology therefore has to come from something else an
 * unmodified client can send. Two discriminators are supported:
 * <ol>
 ...
 * ({@code ServiceRegistry.lookupServiceURL}), so a gRPC backend is declared in
 * topology XML like any other service. The registry treats service
...
public class RoutingInterceptor implements ServerInterceptor {
  ...
  private final Supplier<String> defaultTopology;
  private final GatewayServices services;
  private final String serviceRole;
  private final Metadata.Key<String> topologyKey;

  /**
   * @param defaultTopology supplies the topology to use when a client selects
   *        none. Read per call rather than captured, both so a changed default
   *        applies without a restart and so each listener has its own.
   * @param services the gateway services, for the backend registry
   * @param serviceRole the role a backend is declared under
   * @param topologyKey the metadata entry clients select a topology with
   */
  public RoutingInterceptor(Supplier<String> defaultTopology, GatewayServices services,
                            String serviceRole, Metadata.Key<String> topologyKey) {
