...
public interface MessageInterceptor<T> {
  ...
  /**
   * Returns an interceptor that applies this one and then the given one, so a
   * per-RPC check and a rewrite that applies to everything can be composed
   * without either knowing about the other.
   *
   * @param next applied to this interceptor's result
   * @return the combined interceptor
   */
  default MessageInterceptor<T> andThen(MessageInterceptor<T> next) {
    return message -> next.intercept(intercept(message));
  }
}
