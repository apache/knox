...
public class KnoxCacheManager implements org.apache.shiro.cache.CacheManager, Initializable, Destroyable {
  ...
  private org.ehcache.config.Configuration resolveLockConflict(org.ehcache.config.Configuration configuration) {
    ...
    if (serviceConfig.isPresent()) {
      ...
      // ehcache 3.10 (jakarta) exposes getServiceCreationConfigurations() as an
      // unmodifiable collection, so derive a new configuration with the
      // persistence service replaced rather than mutating the existing one.
