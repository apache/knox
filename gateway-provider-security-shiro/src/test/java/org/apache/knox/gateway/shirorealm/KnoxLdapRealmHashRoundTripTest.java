...
public class KnoxLdapRealmHashRoundTripTest {

  /** Exposes the protected createAuthenticationInfo for testing. */
  private static class TestableRealm extends KnoxLdapRealm {
    AuthenticationInfo build(AuthenticationToken token) throws Exception {
      return createAuthenticationInfo(token, token.getPrincipal(), token.getCredentials(), (LdapContext) null);
    }
  }

  @Test
  public void correctPasswordMatchesUnderShiro221() throws Exception {
  ...
  @Test
  public void wrongPasswordIsRejectedUnderShiro221() throws Exception {
    TestableRealm realm = new TestableRealm();
    UsernamePasswordToken stored = new UsernamePasswordToken("alice", "s3cr3t");
    AuthenticationInfo info = realm.build(stored);
