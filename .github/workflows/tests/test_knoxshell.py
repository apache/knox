...
"""Integration tests for the KnoxShell command-line client."""
...
class TestKnoxShell(unittest.TestCase):
    """Exercise KnoxShell against a running Knox gateway."""
    ...
    def test_1_build_trust_store(self):
        ...
        self.assertEqual(
            result.returncode, 0,
            f"buildTrustStore failed with return code {result.returncode}"
        )
    ...
    def test_2_init_and_list_token(self):
        ...
        self.assertEqual(
            ...
            f"init failed with return code {init_result.returncode}"
        )
        ...
        self.assertEqual(
            ...
            f"list failed with return code {list_result.returncode}"
        )
        self.assertIn(
            "knoxldap", list_result.stdout,
            "Token for knoxldap not found in list output"
        )
