...
class NiFiRegistryRequestUtil {
  ...
  static HttpUriRequest modifyOutboundRequest(HttpUriRequest outboundRequest, HttpServletRequest inboundRequest) throws IOException {
    ...
    final Header originalXForwardedContextHeader = outboundRequest.getFirstHeader(NiFiRegistryHeaders.X_FORWARDED_CONTEXT);
    if (originalXForwardedContextHeader != null) {
      ...
      if (xForwardedContextHeaderValue != null && !xForwardedContextHeaderValue.isEmpty()) {
        ...
        // rules that needs to be added to the X-Forwarded-Context header to allow proper proxying to NiFi Registry.
        //
        // NiFi Registry does its own URL rewriting, and will not work with the context path provided by Knox
        // (ie, "/gateway/sandbox").
        //
        // For example, if Knox has a rewrite rule "*://*:*/**/nifi-registry-app/{**}?{**}", "/nifi-registry-app" needs to be added
        // to the existing value of the X-Forwarded-Context header, which ends up being "/gateway/sandbox/nifi-registry-app".
        ...
        if (index >= 0) {
          ...
          Logger.getLogger(NiFiRegistryHaDispatch.class.getName()).error(String.format(Locale.ROOT, "Unable to find index of %s in %s", outboundRequestUriPathNoTrailingSlash, inboundRequestPathInfo));
        }
        outboundRequest.setHeader(NiFiRegistryHeaders.X_FORWARDED_CONTEXT, xForwardedContextHeaderValue + knoxRouteContext);
      }
    }

    // NiFi Registry requires the header "X-ProxiedEntitiesChain" to be set with the identity or identities of the authenticated requester.
    ...
    String proxiedEntitesChainHeader = inboundRequest.getHeader(NiFiRegistryHeaders.X_PROXIED_ENTITIES_CHAIN);
    if(proxiedEntitesChainHeader == null) {
      proxiedEntitesChainHeader = "";
    }
    outboundRequest.setHeader(NiFiRegistryHeaders.X_PROXIED_ENTITIES_CHAIN, proxiedEntitesChainHeader +
        String.format(Locale.ROOT, "<%s>", "anonymous".equalsIgnoreCase(effectivePrincipalName) ? "" : effectivePrincipalName));

    // Make sure headers named "Cookie" are removed from the request to NiFi Registry, since NiFi Registry does not use cookies.
