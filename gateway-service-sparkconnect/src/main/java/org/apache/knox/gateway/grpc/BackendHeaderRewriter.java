 ...
 * meaning beyond it, so it is removed rather than forwarded. In its place, if
 * the backend is configured with a pre-shared token
 * ({@code spark.connect.authenticate.token}), Knox presents that. Besides
 * authenticating the gateway to Spark, it closes the hole where a client with
 * network reachability to the backend port could simply bypass the gateway
