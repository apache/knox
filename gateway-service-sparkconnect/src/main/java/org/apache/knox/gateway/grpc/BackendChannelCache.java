 ...
 * {@code gateway.sparkconnect.channel.idle.timeout} and reconnect transparently
...
public class BackendChannelCache {
  ...
  private ManagedChannel createChannel(String backendUrl) {
    ...
    if (host == null || port < 0) {
      throw Status.FAILED_PRECONDITION
          .withDescription("Spark Connect backend URL must include a host and port: " + backendUrl)
          .asRuntimeException();
    }
    ...
    if (TLS_SCHEME.equals(scheme)) {
      ...
      try {
        ...
        throw Status.UNAVAILABLE
            .withDescription("Cannot establish TLS to the Spark Connect backend")
            .withCause(e)
            .asRuntimeException();
      }
      ...
      throw Status.FAILED_PRECONDITION
          .withDescription("Spark Connect backend URL scheme must be grpc:// or grpcs://, got: " + backendUrl)
          .asRuntimeException();
    }
  ...
  private static URI parse(String backendUrl) {
    try {
      ...
      throw Status.FAILED_PRECONDITION
          .withDescription("Malformed Spark Connect backend URL: " + backendUrl)
          .withCause(e)
          .asRuntimeException();
    }
  }
