 ...
 * all a vanilla Spark Connect client can carry: the {@code sc://} connection
 * string offers a {@code token=} parameter, static metadata and TLS, and gRPC
 * has no challenge-response step for SPNEGO to hook into. In a Kerberos
 * deployment the user still authenticates with Kerberos — to the {@code
 * knoxtoken} API, over HTTPS — and the resulting JWT acts as the delegation
 * credential on the data path, exactly as delegation tokens do for HDFS.
 * <p>
...
public class TokenAuthenticator {
  ...
  private static final String CACHE_NAME = "sparkconnect";
