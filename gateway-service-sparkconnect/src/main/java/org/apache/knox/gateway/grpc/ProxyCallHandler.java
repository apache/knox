 ...
 * no need for a handler per shape, and Spark Connect's long-lived
 * {@code ExecutePlan} streams work by the same code path as a unary
 * {@code Config}.
 * <p>
 * Backend {@link Status} and trailers are relayed verbatim. This matters more
 * than it might appear: gRPC carries {@code grpc-status} in trailers, and Spark
 * Connect packs {@code google.rpc.ErrorInfo} in there too, so anything that
 * interprets or drops trailers breaks error reporting wholesale.
 * <p>
