...
public class KnoxCLITest {
  ...
  public void testRemoteConfigurationRegistryClientService() throws Exception {
    ...
    Configuration config = createGatewayConfig();
    ...
    cli.run(new String[]{"version", "--master", "master"});
  ...
  public void testRemoteConfigurationRegistryGetACLs() throws Exception {
    ...
    final File testRoot = testFolder.newFolder();
  ...
  public void testSuccessfulAliasLifecycle() throws Exception {
    ...
    int rc;
    KnoxCLI cli = new KnoxCLI();
    cli.setConf(createGatewayConfig());
    rc = cli.run(args1);
    assertEquals(0, rc);
  ...
  public void testGatewayAndClusterStores() throws Exception {
    GatewayConfigImpl config = createGatewayConfig();
  ...
  private void createTestMaster(Configuration config) throws Exception {
    outContent.reset();
  ...
  private GatewayConfigImpl createGatewayConfig() throws IOException {
    GatewayConfigImpl config = new GatewayConfigImpl();
    config.set("gateway.data.dir", testFolder.newFolder().getAbsolutePath());
    return config;
  }
}
